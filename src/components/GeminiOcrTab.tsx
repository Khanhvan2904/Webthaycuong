...
import { UploadCloud, FileText, Sparkles, Download, AlertCircle, CheckCircle2, Loader2, ChevronRight, FileDown, Image as ImageIcon, Layers, Zap, RefreshCw } from 'lucide-react';
import ReactMarkdown from 'react-markdown';
import { convertPdfToImages, getPdfPageCount } from '../utils/pdfUtils';
import { ocrAllPages, combinePageResults, GeminiOcrPageResult } from '../services/geminiOcrService';
...
export const GeminiOcrTab: React.FC = () => {
  ...
  const handleStart = useCallback(async () => {
    ...
    if (!apiKey) {
      setErrorMsg('Chưa tìm thấy API Key. Vui lòng thêm GEMINI_API_KEY vào biến môi trường.');
      setStatus('error');
      return;
    }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              {pageStates.length > 0
                ? `${completedPages}/${pageStates.length} trang xong`
                : 'Đang khởi tạo...'}
            </p>
          </div>
        </div>
              ...
              {pageStates.map((page, idx) => (
                ...
                >
                  <div className="shrink-0">
                    ...
                    {page.status === 'pending' && <div className="w-4 h-4 rounded-full border-2 border-slate-200" />}
                  </div>
